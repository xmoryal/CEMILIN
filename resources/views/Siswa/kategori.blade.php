...
<body class="p-5">
    <section class="container mt-3">
      <div class="row g-4 p-3" id="product-grid">
        <!-- Kolom pencarian -->
      <div>
        <form class="d-flex" role="search" method="GET" action="{{ route('siswa.produk.kategori', request()->route('id')) }}">
          ...
          @if (request()->filled('search'))
            <a href="{{ route('siswa.produk.kategori', request()->route('id')) }}" class="btn btn-outline-secondary">Reset</a>
          @endif
        </form>
      </div>
