...
<body class="p-5">
    <section class="container mt-3">
        <h1 class="ms-3">kantin {{ $namaKantin }}</h1>
      <div class="row g-4 p-3" id="product-grid">
        <!-- Kolom pencarian -->
      <div>
        <form class="d-flex" role="search" method="GET" action="{{ route('siswa.produk.kantin', request()->route('id')) }}">
          ...
          @if (request()->filled('search'))
            <a href="{{ route('siswa.produk.kantin', request()->route('id')) }}" class="btn btn-outline-secondary">Reset</a>
          @endif
        </form>
      </div>
        ...
        @foreach ($produk as $produks)
          ...
          <article class="card product-card h-100">
            ...
            <div class="card-body d-flex flex-column">
              ...
              <p class="card-text small mb-3">{{ $produks->deskripsi }}</p>
